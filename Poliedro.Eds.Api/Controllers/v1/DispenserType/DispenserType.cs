...
namespace Poliedro.Eds.Api.Controllers.v1.DispenserType;

[Route("api/v1/dispenser-type")]
[ApiController]
public class DispenserTypeController(IMediator mediator) : ControllerBase
{
    /// <summary>
    /// Retrieves all DispenserType records.
    /// </summary>
    ...
    {
        var data = await mediator.Send(new GellAllDispenserTypeQuery(new PaginationParams { PageNumber = paginationParams.PageNumber, PageSize = paginationParams.PageSize }));
        if (data is null)
        {
    ...
    [SwaggerResponse(StatusCodes.Status404NotFound, "The specified DispenserType does not exist.", typeof(ProblemDetails))]
    ...
    [SwaggerResponse(StatusCodes.Status400BadRequest, "Incorrect request parameters.", typeof(ProblemDetails))]
    [SwaggerResponse(StatusCodes.Status401Unauthorized, "The request lacks valid authentication credentials.", typeof(ProblemDetails))]
    [SwaggerResponse(StatusCodes.Status404NotFound, "The requested DispenserType was not found.", typeof(ProblemDetails))]
    [SwaggerResponse(StatusCodes.Status500InternalServerError, "Error processing the request.", typeof(ProblemDetails))]
    [Authorize(Policy = "AdminOnly")]
    [HttpPut]
 ...
 [FromServices] IValidator<UpdateDispenserTypeCommand> validator)
    {
        ...
        if (!result.IsSuccess)
        {
            if (result.Error is DispenserTypeErrorBuilder)
            {
