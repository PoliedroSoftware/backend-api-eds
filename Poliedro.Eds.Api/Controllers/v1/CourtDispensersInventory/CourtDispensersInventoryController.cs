...
namespace Poliedro.Eds.Api.Controllers.v1.CourtDispensersInventory
{
    [Route("api/v1/court-dispensers-inventory")]
    [ApiController]
    public class CourtDispensersInventoryController(IMediator mediator) : ControllerBase
    {
        [Authorize(Policy = "AdminOnly")]
        [HttpGet]
        public async Task<IActionResult> GetAll([FromQuery] PaginationParams paginationParams)
        {
            var data = await mediator.Send(new GellAllCourtDispensersInventoryQuery(new PaginationParams { PageNumber = paginationParams.PageNumber, PageSize = paginationParams.PageSize }));
            if (data is null)
            {
        ...
        [SwaggerOperation(Summary = "Get CourtDispensersInventory")]
        [SwaggerResponse(StatusCodes.Status200OK, "The operation was successful.", typeof(CourtDispensersInventoryDto))]
        [SwaggerResponse(StatusCodes.Status400BadRequest, "Incorrect request parameters.", typeof(ProblemDetails))]
        [SwaggerResponse(StatusCodes.Status401Unauthorized, "The request lacks valid authentication credentials.", typeof(ProblemDetails))]
        [SwaggerResponse(StatusCodes.Status404NotFound, "The specified CourtDispensersInventory does not exist.", typeof(ProblemDetails))]
        ...
        [SwaggerOperation(
            Summary = "Create new CourtDispensersInventory")]
        ...
        [SwaggerOperation(Summary = "Update an existing CourtDispensersInventory")]
        [SwaggerResponse(StatusCodes.Status200OK, "The operation was successful.")]
        [SwaggerResponse(StatusCodes.Status400BadRequest, "Incorrect request parameters.", typeof(ProblemDetails))]
        [SwaggerResponse(StatusCodes.Status401Unauthorized, "The request lacks valid authentication credentials.", typeof(ProblemDetails))]
        [SwaggerResponse(StatusCodes.Status404NotFound, "The requested CourtDispensersInventory was not found.", typeof(ProblemDetails))]
        [SwaggerResponse(StatusCodes.Status500InternalServerError, "Error processing the request.", typeof(ProblemDetails))]
        [Authorize(Policy = "AdminOnly")]
        [HttpPut]
     ...
     [FromServices] IValidator<UpdateCourtDispensersInventoryCommand> validator)
        {
            ...
            if (!result.IsSuccess)
            {
                if (result.Error is CourtDispensersInventoryErrorBuilder)
                {
