...
namespace Poliedro.Eds.Api.Tests.IntegrationTests.ProductType;
...
[Fact]
    ...
    {
 var response = await _client.GetAsync("/api/v1/producttype?PageNumber=1&PageSize=10");
    ...
    {
  var response = await _client.GetAsync("/api/v1/producttype/1");
 ...
 {
        var data = new { Name = "Test Product Type", Description = "Test Description" };
        var content = new StringContent(JsonSerializer.Serialize(data), Encoding.UTF8, "application/json");
 var response = await _client.PostAsync("/api/v1/producttype", content);
      Assert.NotNull(response);
    }
}
