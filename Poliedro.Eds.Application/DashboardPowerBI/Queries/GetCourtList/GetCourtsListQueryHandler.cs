    ...
    IMapper mapper) : IRequestHandler<GetCourtsListQuery, IEnumerable<Dtos.Court.CourtListResponseDto>>
{
    public async Task<IEnumerable<Dtos.Court.CourtListResponseDto>> Handle(GetCourtsListQuery request, CancellationToken cancellationToken)
    {
        ...
        var cachedData = await redisService.GetCacheAsync<IEnumerable<Dtos.Court.CourtListResponseDto>>(cacheKey);
        if (cachedData != null)
            return cachedData;

        var entities = await courtListDomainService.GetAllAsync(paginationParams, username!, isAdmin);
        var dtos = mapper.Map<IEnumerable<Dtos.Court.CourtListResponseDto>>(entities);
